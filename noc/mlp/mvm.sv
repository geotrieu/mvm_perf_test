// This MVM version does not use AXIS tuser.
...
module mvm # (
	...
	output [DATAUSERW-1:0] axis_tx_tdata,
...
// Extract the tuser from the expanded tdata
wire [DATAW-1:0] tdata;
wire [USERW-1:0] tuser;
assign tdata = axis_rx_tdata[DATAW-1:0];
assign tuser = axis_rx_tdata[DATAUSERW-1:DATAW];
...
assign tuser_rf_addr 		= tuser[8:0];
assign tuser_op 				= tuser[10:9];
assign tuser_rf_en 			= tuser[74:11];
...
for (dpe_id = 0; dpe_id < DPES; dpe_id = dpe_id + 1) begin: generate_datapath
	memory_block # (
		.DATAW(DATAW),
		.DEPTH(RFDEPTH),
		.MEM_INIT_HEX($sformatf("%s_dot%0d.hex", MEM_INIT_FILE_PREFIX, dpe_id))
	) rf (
		.clk(clk),
		.rst(rst),
		.waddr(rf_waddr),
...
always @ (posedge clk) begin
	if (axis_rx_tvalid && axis_rx_tready) begin
		if (tuser_op == 0) begin
			inst_init_idata <= tdata[INSTW-1:0];
